...
def apology(top="", bottom=""):
    ...
    def escape(s):
        ...
        for old, new in [("-", "--"), (" ", "-"), ("_", "__"), ("?", "~q"),
            ...
            s = s.replace(old, new)
        return s
    return render_template("apology.html", top=escape(top), bottom=escape(bottom), title = top + " " + bottom)
...
def index():
    ...
    user = cur.fetchone()
    try:
        worden = request.args['word'].lower()
        ...
        cur.execute("INSERT INTO words (userID, word, time, wordar) VALUES (%s , %s, %s, %s)",
                    (session["user_id"], worden, datetime.date.today(), wordsar[0].encode('utf-8')))
        db.commit()

        # os.system("ffmpeg -f image2 -r 1/10 -start_number 1 -i %05d.png -vframes 3 -vcodec mpeg4 -y movie.mp4")

        return render_template("index.html",title = worden.capitalize() , wordsar=wordsar, wordsen=wordsen, languages=languages, booa = True)


    except:
        wordar = "عربى"
        return render_template("index.html", wordar = wordar.decode("utf-8"),worden= "english", userimg=user[7], languages=languages, booa = False)
...
def register():
    ...
    if request.method == "POST":
        ...
        if cur.rowcount == 0:
            ...
            cur.execute("INSERT INTO users (username, hash, imgLink, age, gender, firstLang, email) VALUES (%s , %s, %s, %s, %s, %s, %s)",
                        (request.form.get("username"), hash, filename, request.form.get("age"), request.form.get("gender"), request.form.get("lang"), request.form.get("email")))
            db.commit()
            cur.execute("SELECT * FROM users WHERE username = %s",
                        [request.form.get("username")])
            rows = cur.fetchall()
...
def profile():
    ...
    cur.execute("SELECT * FROM words WHERE userID = %s", [session["user_id"]])
    words = cur.fetchall()
    ...
    return render_template("profile.html", title = user[1] ,name = user[1], email = user[1] + "@Banana.com", age = user[3], gender=user[4], IQ=user[8], userimg=user[7], words = wordsqq)
